...
slackApp.command('/phish-a-friend', async ({ command, ack, client }) => {
  ...
  const orgResult = await db.query<{ id: string; peer_phish_enabled: boolean }>(
  ...
  if (!org?.peer_phish_enabled) {
    await client.chat.postEphemeral({
      ...
      text: 'Peer Phish is not enabled for your organization. Contact your admin to turn it on.',
    })
    return
  }

  const templates = await db.query<{ id: string; name: string; difficulty: string }>(
    'SELECT id, name, difficulty FROM phish_templates WHERE active = true ORDER BY difficulty'
  )

  const targets = await db.query<{ id: string; display_name: string | null; provider_id: string }>(
      ...
      blocks: [
        {
          ...
          text: {
            type: 'mrkdwn',
            text: '⚠️ By sending a phishing simulation you agree to the *Peer Phish Terms of Use*. All simulations are logged with your user ID and timestamp.',
          },
        },
        {
          type: 'input',
          ...
          element: {
            ...
            options: templates.rows.map((t) => ({
              text: { type: 'plain_text' as const, text: `${t.name} (${t.difficulty})` },
              value: t.id,
            })),
          },
        },
        {
          ...
          element: {
            ...
            options: targets.rows.map((u) => ({
              text: { type: 'plain_text' as const, text: u.display_name ?? u.provider_id },
              value: u.id,
            })),
          },
        },
