...
export default async function SetupPage() {
  ...
  async function saveSetup(_prev: SetupState, formData: FormData): Promise<SetupState> {
    ...
    if (orgId) {
      ...
      const res = await fetch(`${env.API_URL}/api/orgs`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: current.user.email,
          display_name: current.user.name ?? current.user.email,
          timezone,
      ...
      const { userId, orgId: newOrgId, role } = await res.json();
      await unstable_update({ user: { id: userId, orgId: newOrgId, role } });
      redirect('/dashboard');
    }
  }
